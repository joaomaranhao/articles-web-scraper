...
def rewrite_paragraph(paragraph: str, pt_br: bool = True) -> str | None:
    ...
    try:
        if pt_br:
            ...
            time.sleep(0.5)
            language_button = language_dropdown.find_element(
                By.XPATH, f"//*[text()='{language_value}']"
            )
            time.sleep(0.5)
        ...
        rewrite_button = scraper.driver.find_element(
            By.XPATH, f"//*[@aria-label='{button_aria_label}']"
        )
        time.sleep(0.5)
